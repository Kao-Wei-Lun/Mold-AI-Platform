...
| 階段 | 交付 | 提交前測試 | 狀態 |
| --- | --- | --- | --- |
| 0 | 本規劃與契約 | 檢視對應程式／需求、diff check | 規劃建立 |
| 1 | CPU 多方向雙向比對核心、合成控制與資源邊界 | 核心新測試、既有幾何回歸、lint | 待開發 |
| 2 | 工作政策快照、搜尋重排、故障分級、結果契約與中英 UI | API/Worker、UI、完整 test.ps1 | 待開發 |
| 3 | 多尺度局部證據、STEP 結構特徵、尺寸模式 | 避免網格密度偏差、單位與結構反例 | 待開發 |
| 4 | 擴充公開 corpus、人工標註工具、分數校準／無匹配 | 獨立 holdout 品質報告 | 待開發，需人工判讀 |
| 5 | Persistent cache、召回／延遲調優、外網 UAT | cold/warm 實測、私人入口與版本校驗 | 待開發 |
