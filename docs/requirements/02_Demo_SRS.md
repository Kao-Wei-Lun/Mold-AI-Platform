...
- **D-KNW-004**：知識能力必須依使用目的分成兩個工作區。「工程分析／工程知識搜尋」只負責提問、條件篩選、證據結果、引用與來源跳轉；「治理管理／知識文件管理」只負責文件匯入、清單、版本、索引狀態與發布生命週期，不得再以左右欄混合兩種工作。
- **D-KNW-005**：文件匯入使用獨立操作畫面；匯入表單下方不得混入既有文件清單。匯入成功後應提供「查看匯入文件」與「返回文件管理」兩個明確後續動作。
- **D-KNW-006**：工作流程理由只在使用者選擇送審、核准、發布或退役後，於情境式確認對話框中要求輸入；對話框必須說明狀態轉換與影響，理由需寫入稽核軌跡。
- **D-KNW-007**：知識文件 UI 應以「停止發布／已停止發布」表達後端 `retire/retired` 狀態，避免使用者將「退役」誤解為刪除。停止發布按鈕必須以明確但非主要操作的警示樣式呈現；匯入與搜尋表單的主要按鈕應橫跨表單寬度，欄位需由頂端對齊。
