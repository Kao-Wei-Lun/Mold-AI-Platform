...
<template>
  <section class="ingestion-workspace" aria-labelledby="ingestion-title">
    ...
    <section v-if="canCreate" class="ingestion-create-panel">
      ...
      <form class="ingestion-create-grid" @submit.prevent="startImport"><FormField v-slot="{ fieldId }" :label="t('Data type')" required><select :id="fieldId" v-model="form.domain"><option value="master_data">{{ t("Engineering reference data") }}</option><option value="projects">{{ t("Projects") }}</option><option value="registry">{{ t("Project, part, mold and revision registry") }}</option><option value="rule_profiles">{{ t("Mold rule profiles") }}</option></select></FormField><FormField v-slot="{ fieldId }" :label="t('Source name')"><input :id="fieldId" v-model="form.sourceName" /></FormField><FormField v-slot="{ fieldId }" :label="t('Idempotency key')" required><input :id="fieldId" v-model="form.idempotencyKey" required /></FormField><FormField v-slot="{ fieldId }" :label="t('Source file')" required hint="JSON · CSV · XLSX"><input :id="fieldId" type="file" accept=".json,.csv,.xlsx" required @change="onFile" /></FormField><button type="submit" :disabled="busy">{{ t("Upload and screen") }}</button></form>
    </section>
