...
async function loadDocuments(): Promise<void> {
  ...
  try {
    ...
    error.value = caught instanceof Error ? caught.message : "Unable to load knowledge documents.";
  } finally {
...
async function refreshJob(): Promise<void> {
  ...
  try {
    ...
    error.value = caught instanceof Error ? caught.message : "Unable to refresh ingestion job.";
  }
}

async function submitUpload(): Promise<void> {
  if (!file.value) {
    error.value = "Choose a UTF-8 TXT or Markdown file first.";
    return;
  }
  ...
  try {
    ...
    error.value = caught instanceof Error ? caught.message : "Knowledge upload failed.";
  } finally {
...
async function submitSearch(): Promise<void> {
  ...
  try {
    ...
    error.value = caught instanceof Error ? caught.message : "Knowledge search failed.";
  } finally {
  ...
  <section id="knowledge" class="knowledge-workspace" aria-labelledby="knowledge-title">
    <div class="section-heading">
      <div>
        ...
        <h2 id="knowledge-title">Retrieve governed source evidence</h2>
      </div>
      <span class="demo-label">Extractive · No LLM Synthesis</span>
    </div>
      ...
      <article class="knowledge-ingestion">
        <div class="subsection-heading">
          <div>
            ...
            <span>{{ indexedCount }} indexed · Public Demo ACL</span>
          </div>
        ...
        <form class="knowledge-upload-form" @submit.prevent="submitUpload">
          <label class="file-field">
            <span>UTF-8 TXT or Markdown</span>
            <input type="file" accept=".txt,.md,text/plain,text/markdown" @change="onFile" />
          </label>
          <label>
            <span>Title</span>
            <input v-model="title" type="text" maxlength="255" placeholder="Demo molding SOP" />
          </label>
          <label>
            <span>Document type</span>
            <select v-model="documentType">
              <option value="demo_sop">Demo SOP</option>
              <option value="design_guideline">Design guideline</option>
              <option value="trial_report">Trial report</option>
          ...
          <label>
            ...
            <select v-model="authorityLevel">
              ...
              <option value="reviewed_demo">Reviewed Demo</option>
            </select>
          </label>
          <label>
            <span>Language</span>
        ...
        <div v-if="job" class="job-panel compact-job">
          ...
          <div class="progress-track" aria-label="Knowledge ingestion progress">
            <span :style="{ width: `${job.progress}%` }"></span>
          </div>
        </div>

        <ul class="document-list" aria-label="Knowledge documents">
          <li v-for="document in documents.slice(0, 8)" :key="document.document_id">
            <div>
              <strong>{{ document.title }}</strong>
              <span>{{ document.document_type.replaceAll("_", " ") }} · {{ document.chunk_count }} chunks</span>
            </div>
      ...
      <article class="knowledge-search">
        ...
        <form class="knowledge-search-form" @submit.prevent="submitSearch">
          <label class="query-field">
            <span>Engineering question or terms</span>
            <textarea
              v-model="query"
              rows="3"
              maxlength="500"
              placeholder="What does the source say about rib thickness?"
              required
            ></textarea>
          </label>
          <label>
          ...
          <button type="submit" :disabled="searching || indexedCount === 0">
            {{ searching ? "Retrieving..." : "Search authorized evidence" }}
          </button>
        </form>
        ...
        <template v-if="searchResult">
          ...
          <div v-if="selectedResult" class="knowledge-evidence">
            <div class="evidence-score-row">
              <strong>Evidence #{{ selectedResult.rank }}</strong>
